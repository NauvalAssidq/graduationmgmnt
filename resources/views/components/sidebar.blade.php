<aside class="w-64 bg-emerald-800 border-r border-emerald-700 flex-shrink-0 fixed h-full z-10 hidden lg:block">
    ...
    <nav class="p-4 space-y-1">
        ...
        <div class="pt-4 pb-2">
            <p class="px-3 text-[11px] font-semibold text-emerald-400 uppercase tracking-wider">Sistem</p>
        </div>

        <a href="{{ route('kelola-admin.index') }}" class="flex items-center gap-3 px-3 py-2.5 {{ request()->routeIs('kelola-admin.*') ? 'bg-emerald-900 text-white' : 'text-emerald-100 hover:text-white hover:bg-emerald-700' }} text-sm font-medium rounded-md transition-colors">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"></path></svg>
            Kelola Admin
        </a>
        @else
        ...
        <a href="{{ route('settings.index') }}" class="flex items-center gap-3 px-3 py-2.5 {{ request()->routeIs('settings.index') ? 'bg-emerald-900 text-white' : 'text-emerald-100 hover:text-white hover:bg-emerald-700' }} text-sm font-medium rounded-md transition-colors">
